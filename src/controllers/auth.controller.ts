  ...
  try {
    ...
    const createdUserData = await prisma.user.create({
      data: {
        ...req.body,
        roles: {
          connect: req.body.roles.map(res => {
            return { id: res };
          })
        },
    ...
    res.status(201).json({
      data: pick(['id', 'first_name', 'last_name', 'email', 'address', 'roles'], createdUserData),
      message: 'created'
    });
  } catch (error) {
    console.log(error);
...
authRouter.get('/me', async (req: RequestModified, res: Response) => {
  ...
  const userNoPassword = pick(['id', 'first_name', 'last_name', 'address', 'email'], req.user);
...
function createToken(user: User): TokenData {
  const expiresIn = Number(config.get('jwt.expired'));
